...
As part of leaving the European Union the UK is building replacement capabilities which will be in place before we withdraw. UK regulators currently use two EU systems ([ICSMS](https://webgate.ec.europa.eu/icsms/) and [RAPEX](https://ec.europa.eu/consumers/consumers_safety/safety_products/rapex/alerts/repository/content/pages/rapex/index_en.htm)) to do market surveillance activities and to make sure that dangerous or unsafe products are not being sold to consumers.

I was part of a team working with the [Office for Product Safety and Standards](https://www.gov.uk/government/organisations/office-for-product-safety-and-standards) (OPSS) within the [Government Department for Business, Energy and Industrial Strategy](https://www.gov.uk/government/organisations/department-for-business-energy-and-industrial-strategy) (BEIS), to build one system which will give an equivalent capability to both previously used systems. 

The UK needs to collect and manage data on product safety and market surveillance from multiple sources and make that data available to UK regulators, enforcement bodies, businesses and consumers through appropriate channels. This service also needs to help manage product safety cases through initial submission to resolution and reporting and keep parties notified of progress.





This was by far the largest team I had worked on, below is a list of the roles within our team and some of their responsibilities.

* 1 product owner
  * Liaising with stakeholders and prioritising development tasks
* 1 project manager
  * Responsible for managing the team and for reporting on project progress
* 1 scrum master
  * Clearing obstacles and upholding Scrum practices like the Daily Scrum and Sprint Retrospectives
* 2 user researchers
...
* 1 technical lead
  * Unblocking developers and reviewing code
* 6 developers
...
Ruby is an object-oriented programming (OOP) language, which meant that we were able to leverage OOP paradigms like inheritance and polymorphism. A key place where inheritance was particularly useful was the activity timeline. On the site we had pages for investigations into products and it was important to have a clear audit trail for the investigation. 

![Audit Activities](/img/mspsds/auditActivities.png)

The activity timeline allowed the user to see everything related to the case, in chronological order. Every entry on the timeline was an `Activity` object, which itself is a subclass of the `ApplicationRecord` class, an [Active Record Model](https://guides.rubyonrails.org/active_record_basics.html#creating-active-record-models) from the Rails framework. 

Different types of activity then inherit from `Activity` such as `AuditActivity` and `CommentActivity` allowing them to share functionality around formatting, timestamping and displaying images. The next level down is for different types of `AuditActivity` classes, dealing with entities such as Businesses, Products or Investigations, which all have their own properties. Finally, for each of these entities we had different functionality for adding, updating and deleting.
...
This inheritance pattern allowed us to share as much code as possible for common features at each level, whilst still being able to have different functionality as an end result.
